...
flowchart LR
  ...
  subgraph DAILY["② 평일 2회 장중 자동 · GitHub 서버 (cron)"]
    direction LR
    C["⏰ 🇰🇷 14:10 · 🇺🇸 23:30 (장중)"] --> D["체크아웃"] --> E["준비 스크립트<br/>데이터·차트 등"] --> F["🧠 Claude 분석·리포트"] --> G["커밋·push"] --> H["🌐 사이트 갱신"]
  end
...
| ✅ | **자정 넘김** | 🇺🇸 세션이 자정을 넘기며 주문서 12건이 조용히 유실 → **회차 기준일(`RUN_DATE`) 고정** (RULES §4-1) |
| ✅ | **조용한 실패** | 초록불인데 결과물이 비어 있던 문제 → **커밋 전 산출물 점검** + 실패 시 이슈 자동 생성 |
| ✅ | **발행 지연** | 순차 실행으로 발행이 15:27까지 밀림 → 매니저 4인 **병렬 실행** + 시작 14:10으로 당김 |
| ✅ | 코드 신선도 | 체크아웃이 대기 '전'이라 3시간 전 코드로 돌던 문제 → **대기 후 체크아웃** |
...
평일 2회 **장중**(🇰🇷 14:30 · 🇺🇸 23:45 발행), 여러 소스에서 **숫자**를 모으고(파이썬), **뉴스**는 Claude 웹검색으로 붙여 종합 분석 → **"지금 이 종목 사라/팔아라"** 리포트 + **AI 4인의 가상 1억 매매**를 게시합니다. 리포트를 본 사람이 **30분 안에 같은 가격대로 실제 매수**할 수 있는 시각에 냅니다.

```mermaid
flowchart LR
  Y["yfinance<br/>지수·주가·환율·원자재·코인 (105)"] --> P
  FR["FRED<br/>미 경제지표 (금리·CPI·실업)"] --> P
  FD["FinanceDataReader<br/>한국 전체 (~2766) 종목"] --> P
  WS["🔎 웹검색<br/>뉴스·시황·심리"] --> CL

  P["처리 (Python)<br/>지표계산·차트·스크리너"] --> CL
  CL["🧠 Claude 5명 (전원 xhigh)<br/>매니저 4인 (opus 2 + fable 2)<br/>→ 애널리스트 종합"] --> R["📄 리포트(.md)<br/>🇰🇷 14:30 · 🇺🇸 23:45 장중"] --> PG["🌐 GitHub Pages"]
...
| 소스 | 수집 내용 | 비고 |
|---|---|---|
| `yfinance` | 미·한·해외 지수, 섹터 ETF 14종, 한·미 주요주 54종, 환율, 채권(TLT·IEF·LQD·HYG), VIX, 원자재 7종, 암호화폐 4종 — **105종목** | 글로벌·안정적 |
...
| 단계 | 스크립트 | 내용 |
|---|---|---|
| 📥 데이터 수집 | `scripts/collect_data.py` | 105종목 시세 + 경제지표(FRED 8종)를 JSON으로 |
...
| 🧑‍💼 Claude ①~④ 매니저 4인 (먼저) | `prompts/persona-*.md` + `portfolio.md` | 🛡️안정·🚀공격(성향 부여한 극단 양 끝, opus) + 🙂평범형 1·2(**완전히 동일한 지시문**의 대조군, fable — 같은 조건에서 판단이 얼마나 갈리는지 측정). 데이터 직접 보고 **종목 매수/매도 확정** (독립 세션·계좌) |
| 🧠 Claude ⑤애널리스트 (뒤에) | `prompts/kr-report.md`·`prompts/us-report.md` | **4인 주문서 종합** → "오늘 이 종목 사라/팔아라" 종목 결론 리포트 + 어제 채점 |
...
> **핵심 철학:** "지금 사면 오를 종목"을 종목 단위로 콜하고, 다음 리포트에서 기준가 대비 O/△/X로 **자기채점**.
> 4계좌의 실손익이 곧 성적표 — AI가 진짜 잘하는지 정직하게 관찰합니다. *(투자 조언 아님)*

---

- 하루 2회 자동 실행 · **장중 발행(고정)**: 🇰🇷 매일 14:30 · 🇺🇸 매일 23:45 (GitHub Actions) · **제1원칙 = 리포트 본 사람이 30분 안에 실제 매수 가능할 것** → 매매는 **AI가 분석한 그 시세로 체결**(리포트에 기준가 표시) · Claude 5명 전원 xhigh(①~④ 매니저 종목 확정 → ⑤ 애널리스트 종합 리포트) · 데이터 = 파이썬
