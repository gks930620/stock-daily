...
flowchart LR
  ...
  subgraph DAILY["② 평일 2회 장중 자동 · GitHub 서버 (cron)"]
    direction LR
    C["⏰ 🇰🇷 14:40 · 🇺🇸 23:30 (장중)"] --> D["체크아웃"] --> E["준비 스크립트<br/>데이터·차트 등"] --> F["🧠 Claude 분석·리포트"] --> G["커밋·push"] --> H["🌐 사이트 갱신"]
  end
...
| ✅ | **발행 지연** | 순차 실행으로 발행이 15:27까지 밀림 → 매니저 4인 **병렬 실행** + 시작 14:40으로 당김 |
...
평일 2회 **장중**(🇰🇷 15:00 · 🇺🇸 23:45 발행), 여러 소스에서 **숫자**를 모으고(파이썬), **뉴스**는 Claude 웹검색으로 붙여 종합 분석 → **"지금 이 종목 사라/팔아라"** 리포트 + **AI 4인의 가상 1억 매매**를 게시합니다. 리포트를 본 사람이 **30분 안에 같은 가격대로 실제 매수**할 수 있는 시각에 냅니다.
...
flowchart LR
  ...
  CL["🧠 Claude 5명 (전원 xhigh)<br/>매니저 4인 (opus 2 + fable 2)<br/>→ 애널리스트 종합"] --> R["📄 리포트(.md)<br/>🇰🇷 15:00 · 🇺🇸 23:45 장중"] --> PG["🌐 GitHub Pages"]
...
- 하루 2회 자동 실행 · **장중 발행(고정)**: 🇰🇷 매일 15:00 · 🇺🇸 매일 23:45 (GitHub Actions) · **제1원칙 = 리포트 본 사람이 30분 안에 실제 매수 가능할 것** → 매매는 **AI가 분석한 그 시세로 체결**(리포트에 기준가 표시) · Claude 5명 전원 xhigh(①~④ 매니저 종목 확정 → ⑤ 애널리스트 종합 리포트) · 데이터 = 파이썬
