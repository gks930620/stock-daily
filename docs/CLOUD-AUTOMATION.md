...
헤드리스 실행은 `claude -p`의 플래그로 **모델**과 **추론 강도(effort)** 를 지정합니다. 이 프로젝트는 **최신 Opus + 최고 effort**로 고정:

```bash
claude -p "$PROMPT" --model opus --effort xhigh ...
```
...
- 이 프로젝트는 `xhigh`(=최고 심층추론, `max`는 과추론 경향이라 제외)
