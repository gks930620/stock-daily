...
- **트리거**: 하루 2회 평일 KST(장중) — **🇰🇷 14:10 분석→14:30 발행** · **🇺🇸 23:30 분석→23:45 발행**. GitHub 예약실행은 **최대 2~3시간 지연**되므로, cron을 목표보다 ~3.5h 일찍(`0 2` / `0 11` UTC) 걸고 워크플로가 목표 시각까지 대기해 **발행 시각을 고정**한다.
...
이게 끝입니다. 이제 **평일 하루 2회**(🇰🇷 14:30 · 🇺🇸 23:45 발행) GitHub 서버가 알아서 실행합니다. 수동 실행 시 모드(kr/us)를 고를 수 있습니다.
...
- **cron은 UTC 기준**(KST = UTC+9). 크론(`0 2`/`0 11`)은 '충분히 일찍' 걸어두는 용도이고, **실제 발행 시각을 정하는 건 `모드 결정` 스텝의 `TARGET`(14:10/23:30)** 입니다. 발행 시각을 바꾸려면 TARGET을 고치고, 크론이 그보다 ~3.5h 이상 이르도록 유지하세요.
