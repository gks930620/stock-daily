...
- **트리거**: 하루 2회 평일 KST(장중) — **🇰🇷 14:30 분석→14:45 발행** · **🇺🇸 23:30 분석→23:45 발행**. cron은 UTC(`45 4` / `0 14`)로 일찍 걸고, 워크플로가 목표 시각까지 대기해 **발행 시각을 고정**한다(GitHub 예약실행은 20~45분 지연됨).
...
- **cron은 UTC 기준**(KST = UTC+9). 발행 시각을 바꾸려면 daily.yml의 cron **과 `모드 결정` 스텝의 `TARGET`** 을 같이 수정하세요 — 실제 시작을 정하는 건 TARGET입니다.
