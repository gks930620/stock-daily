...
- **트리거**: 하루 2회 평일 KST(장중) — **🇰🇷 14:40 분석→15:00 발행** · **🇺🇸 23:30 분석→23:45 발행**. GitHub 예약실행은 **최대 2~3시간 지연**되므로, cron을 목표보다 ~3.5h 일찍(`0 2` / `0 11` UTC) 걸고 워크플로가 목표 시각까지 대기해 **발행 시각을 고정**한다.
...
이게 끝입니다. 이제 **평일 하루 2회**(🇰🇷 15:00 · 🇺🇸 23:45 발행) GitHub 서버가 알아서 실행합니다. 수동 실행 시 모드(kr/us)를 고를 수 있습니다.
...
| 역할 | 세션 수 | effort | 이유 |
|---|---|---|---|
| 🛡️🚀 성향파 2인 (opus) · 🙂🙂 평범형 2인 (fable) | 4 | `xhigh` | 매매 판단의 질이 수익률을 좌우 |
| 🧠 **애널리스트** (4인 종합 리포트) | 1 | `xhigh` | 종합·판정 |

```bash
claude -p "$PROMPT" --model opus --effort xhigh ...   # 5명 전원 (매니저 4인 + 애널리스트)
```
...
- 이 프로젝트: **5명 전원 `xhigh`** (`max`는 과추론 경향이라 제외). Max 사용량 부담 시 매니저만 `high`로 낮추거나 평범형 수를 줄이는 선택지
...
- **cron은 UTC 기준**(KST = UTC+9). 크론(`0 2`/`0 11`)은 '충분히 일찍' 걸어두는 용도이고, **실제 발행 시각을 정하는 건 `모드 결정` 스텝의 `TARGET`(14:40/23:30)** 입니다. 발행 시각을 바꾸려면 TARGET을 고치고, 크론이 그보다 ~3.5h 이상 이르도록 유지하세요.
