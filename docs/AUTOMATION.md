...
- **AI 실행은 하루 2회(kr·us) × 회차당 5세션** (매니저 4 + 애널리스트 1, 전원 xhigh) → Max 구독 사용량으로 처리 (요청당 과금 아님)
  - 모델: 안정형·공격형·애널리스트 = 최신 `opus` / 평범형 1·2 = 최신 `fable` (대조군)
  - ⚠️ 클라우드(`daily.yml`)는 매니저 4인을 **병렬**로 돌린다. 이 로컬 스크립트는 순차라 ~10분 더 걸린다.
