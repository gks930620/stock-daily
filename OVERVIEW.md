...
flowchart LR
  ...
  subgraph DAILY["② 매일 자동 · GitHub 서버 (cron)"]
    direction LR
    C["⏰ 매일 08:00"] --> D["체크아웃"] --> E["준비 스크립트<br/>데이터·차트 등"] --> F["🧠 Claude 분석"] --> G["커밋·push"] --> H["🌐 사이트 갱신"]
  end
...
여러 소스에서 **숫자**를 모으고(파이썬), **뉴스**는 Claude 웹검색으로 붙여 종합 분석 → 확률적 리포트를 게시합니다.
...
flowchart LR
  ...
  CL["🧠 Claude 분석<br/>데이터+뉴스 종합<br/>확률 시나리오·자기검증"] --> R["📄 리포트(.md)<br/>차트 삽입"] --> PG["🌐 GitHub Pages"]

  classDef data fill:#dbf1ee,stroke:#0e8f83,color:#04372f;
  classDef proc fill:#fbeed9,stroke:#bf7318,color:#3a2606;
  classDef ai   fill:#e9e9fe,stroke:#5b5ff0,color:#1c1c4d;
  classDef pub  fill:#e3f6ea,stroke:#219a52,color:#0c3d22;
  class Y,FR,FD data; class P proc; class WS,CL ai; class R,PG pub;
```
...
- 관련 문서: [DESIGN.md](DESIGN.md) · [CLOUD-AUTOMATION.md](CLOUD-AUTOMATION.md) · [AUTOMATION.md](AUTOMATION.md) · [TOOLING.md](TOOLING.md)
