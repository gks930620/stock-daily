...
- **본문 구성(확정 포맷): 결론 → 시나리오 플레이북 → 체크포인트 → 매매일지 → 채점.** 번잡함 금지 — 산문 칼럼 섹션 없이, 아래 구성으로 간결하게. CSS 클래스는 준비돼 있으니 그대로 사용:
  ...
  **(2) 시나리오 플레이북** — `## 🎯 시나리오 플레이북`: 먼저 확률 막대,
  ...
  ```
  이어 시나리오 3개를 각각 `### 🔴/🟡/🔵 시나리오명 (NN%)` + 불릿으로: **근거 / 트리거 / 무효 조건 / 그때의 행동**. 오늘 밤 지표·실적이 있으면 결과별 분기(A면→, B면→)를 트리거에 명시. 판단에 핵심인 차트 1~2개만 이 안에 인라인(각 차트 위 한 줄 해석) — 차트 전용 섹션은 만들지 않는다.
  ...
  **(4) 매매일지** — `## 💼 오늘의 주문` (아래 5번 참고): 주문 표 → **종목별 근거**(산 것마다 2~3문장) → **안 산 이유**(고려했지만 뺀 것 1~2개) → **리스크 관리**(무효 신호 시 행동, 현금 비중 이유).
  ...
  이후: `## ✅ 어제 채점`(O/△/X + 교훈, 오늘 포지션에 어떻게 반영했는지 한 줄).
